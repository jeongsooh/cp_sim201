...
class HardwareAPI:
    """
    물리 장치(GPIO, 센서, 릴레이 등)와의 연동을 추상화하기 위한 더미 API 클래스.
    실제 보드에서는 stm32_hal.py의 STM32HardwareAPI 인스턴스로 교체된다.
    """
    ...
    @staticmethod
    def relay_on(evse_id: int) -> None:
        pass

    @staticmethod
    def relay_off(evse_id: int) -> None:
        pass

    @staticmethod
    def set_cp_pwm(evse_id: int, duty_percent: int) -> None:
        pass
    ...
    @staticmethod
    def read_energy_meter_data(evse_id: int) -> Dict[str, float]:
        """Returns V, I, P, E values from connected serial energy meter."""
        return {"voltage": 0.0, "current": 0.0, "power": 0.0, "energy": 0.0}


class ConnectorHAL:
    def __init__(self, evse_id: int, connector_id: int, ocpp_client: Optional[object] = None) -> None:
...
class TokenReaderHAL:
    def __init__(self, ocpp_client: Optional[object] = None) -> None:
        self.component_name = "TokenReader"
        self.ocpp_client = ocpp_client
        self.enabled = True

    async def on_rfid_scanned(self, raw_data: str) -> None:
        """이벤트: RFID 리더기에서 태그 데이터가 읽혔을 때 호출"""
        if not self.enabled:
            logger.warning("TokenReader is disabled. Ignoring scan.")
            return

        id_token = {"idToken": raw_data, "type": "ISO14443"}
...
class PowerContactorHAL:
    def __init__(self, evse_id: int, ocpp_client: Optional[object] = None) -> None:
        self.evse_id = evse_id
