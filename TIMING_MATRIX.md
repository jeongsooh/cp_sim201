...
| `ocpp_client.py:_tls_protocol_retry_done` | TLS protocol version error 후 one-shot wait_min 재시도 (TC_A_06_CS Phase 2 안착) |
...
| **TC_A_06_CS** | wait_min=90, NetworkProfileConnectionAttempts=1, **OCTT Long Operation Timeout ≥600s** | Phase 1 TLSv1.1 ~65s + Phase 2 TLS 1.2-only cipher window | (1) Reset(Immediate) → systemd restart (true reboot, [[octt-tc-a-06-tls12-cap]]), (2) data/pending_reset.json 복원으로 BootNotification(RemoteReset), (3) **SSL ctx maximum_version=TLS 1.2** (필수 — TLS 1.3 offer 시 Phase 2가 RST), (4) attempt 1 (T+~8s, TLSv1.1 reject via SSLV3_ALERT_HANDSHAKE_FAILURE), (5) attempt 2 (T+~100s, wait_min) → 성공 → BootNotification + SecurityEventNotification(ResetOrReboot/InvalidTLSVersion) | station_config._build_ssl_context (`maximum_version`), controller._execute_reset (systemd restart), persistence.consume_pending_reset, ocpp_client `is_tls_protocol_error` (HANDSHAKE_FAILURE 포함) | **PASS** (2026-05-15, `27b278a`) |
...
| TLS error 후 빠른 재시도 vs exponential doubling | TC_A_06_CS (~90s 필요) vs TC_B_57_CS (180s 이상 doubling 필요) | TLS protocol-version error에 한해 one-shot wait=wait_min; 다른 error는 exponential | TC_A_06이 Phase 1(65s) → Phase 2(좁은 윈도우) 구조라 attempt 2가 ~90s에 떨어져야 함. TC_B_57은 HTTP rejection이라 is_tls_protocol_error=False로 분기 양립 (2026-05-14) |
