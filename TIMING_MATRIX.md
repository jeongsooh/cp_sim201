...
| **TC_A_06_CS** | wait_min=90, NetworkProfileConnectionAttempts=1 | Phase 1 TLSv1.1 ~65s + Phase 2 TLSv1.2 좁은 윈도우 | attempt 1 즉시(skip flag, TLSv1.1 reject) + attempt 2 wait_min(90s, Phase 2 직후 안착) | handle_reset_request, _execute_reset, ocpp_client `is_tls_protocol_error` + `_tls_protocol_retry_done` (wait=wait_min) | **검증 중** (2026-05-14): d819199의 fast-retry 제거가 너무 늦었음(+180s, dead zone hit) → wait=wait_min 절충 |
